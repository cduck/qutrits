...
class PlusKGate(raw_types.TernaryLogicGate,
    ...
    def default_decompose(self, qubits):
        yield from self._gen_partial_carry(qubits, self.k, top=True)

        if self.k[0]:
            yield common_gates.F01(qubits[0])

    def _gen_partial_carry(self, qubits, k, top=True):
        if len(qubits) <= 0: return
            ...
            yield from self._gen_partial_carry(qubits[:half], k[:half], top)
            yield from self._gen_partial_carry(qubits[half:], k[half:], False)
